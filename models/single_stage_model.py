...
import utils
from midas.midas_net import InstaDepthNet_od, InstaDepthNet_d
...
class SingleStageModel(object):
    ...
    def __init__(self, params, dist_model=False):
        ...
        if params['algo'] == "InstaDepthNet_od":
            # get pretrained weight
            self.model = InstaDepthNet_od(params['pretrained_weight'], non_negative=True)
        elif params['algo'] == "InstaDepthNet_d":
            # get pretrained weight
            self.model = InstaDepthNet_d(params['pretrained_weight'], non_negative=True)
        else:
