...
class SupDepthOrderDataset(Dataset):
    ...
    def _get_pair_ind(self, img_id):
        ...
        # if np.sum(gt_depth_matrix[0]) == 0:
        #     return self._get_pair_ind(np.random.choice(len(self)))

        if gt_depth_matrix[0].sum() == gt_depth_matrix[0].shape[0] * gt_depth_matrix[0].shape[1] * (-1):
            return self._get_pair_ind(np.random.choice(len(self)))

        return modal, bboxes, image_fn, None, None, gt_depth_matrix

    def __getitem__(self, idx):
        if self.memcached:
            self._init_memcached()

        img_id, Gorder = self.data_reader.get_imgId_and_depth(idx)
        modal, bboxes, image_fn, _, _, gt_order_overlap_count = self._get_pair_ind(img_id)
        gt_depth_matrix, gt_overlap_matrix, gt_count_matrix = gt_order_overlap_count

        split_char = "<" if "<" in Gorder else "="
        idx1, idx2 = list(map(int, Gorder.split(split_char)))
        ...
        if gt_depth_matrix[idx1, idx2] == -1:
            ...
            print("error")
        # return count and overlap
        count = gt_count_matrix[idx1, idx2]
        is_overlap = gt_overlap_matrix[idx1, idx2]

        if np.random.rand() < 0.5:
            # -1(-1), 0(0), 2(2)
            return rgb, modal_tensor1, modal_tensor2, label, count, is_overlap
        else:
            # -1(-1), 0(1), 2(2)
            label = 1 if label == 0 else label
            return rgb, modal_tensor2, modal_tensor1, label, count, is_overlap
