...
def get_closest_int_multiple_of(orig_num, multiplier):
    if orig_num % multiplier >= multiplier // 2:
        return orig_num + multiplier - (orig_num % multiplier)
    else:
        return orig_num - (orig_num % multiplier)


def inv_transform(rgb):
    inv_normalize = transforms.Normalize(
        mean=[-0.485 / 0.229, -0.456 / 0.224, -0.406 / 0.225],
        std=[1 / 0.229, 1 / 0.224, 1 / 0.225]
    )
    return inv_normalize(rgb)


def transform_rgb(rgb):
    image = torch.from_numpy(rgb.transpose((2, 0, 1)).astype(np.float32)).unsqueeze(0)
...
def transform_resize(image, width, height):
    transform_resize = transforms.Compose(
        [
            ...
            NormalizeImage(mean=data_mean, std=data_std),
            PrepareForNet(),
        ]
    )
    return transform_resize({"image": image / 255.})["image"]
