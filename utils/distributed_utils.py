...
def dist_init_(launcher, backend='nccl', dist_url='tcp://127.0.0.1:1234'):
    if mp.get_start_method(allow_none=True) is None:
        mp.set_start_method('spawn')
    if launcher == 'pytorch':
        ...
        dist.init_process_group(backend=backend, init_method=dist_url, world_size=num_gpus, rank=rank)
...
def gather_tensors_batch(input_array, part_size=10):
    ...
    rank = dist.get_rank()
    all_features = []
    part_num = input_array.shape[0] // part_size + 1 if input_array.shape[0] % part_size != 0 else input_array.shape[
                                                                                                       0] // part_size
    ...
    all_features = [np.concatenate([all_features[i][j] for i in range(part_num)], axis=0) for j in
                    range(len(all_features[0]))]
    return all_features
